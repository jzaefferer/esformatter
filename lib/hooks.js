...
exports.ExpressionStatement = function(node){
    // ExpressionStatement means a lot, this only adds whitespace within `(...)` statements
    ...
    if (end.value !== ')') {
        end = _tk.findInBetween(start, end, function(token) {
            return token.value === ')';
        });
    }
    if (!start || !end) {
    ...
    _ws.afterIfNeeded(start, 'ExpressionStatementOpeningBrace');
    _ws.beforeIfNeeded(end, 'ExpressionStatementClosingBrace');
};
